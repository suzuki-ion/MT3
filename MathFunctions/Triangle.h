...
struct Triangle {
    ...
    /// @return 衝突しているかどうか
    [[nodiscard]] bool IsCollision(const Line &line) const noexcept;

    /// @brief 三角形と半直線の衝突判定
    /// @param ray 衝突判定を行う半直線
    /// @return 衝突しているかどうか
    [[nodiscard]] bool IsCollision(const Ray &ray) const noexcept;

    /// @brief 三角形と線分の衝突判定
    /// @param segment 衝突判定を行う線分
    /// @return 衝突しているかどうか
    [[nodiscard]] bool IsCollision(const Segment &segment) const noexcept;
